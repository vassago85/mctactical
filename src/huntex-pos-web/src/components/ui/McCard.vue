...
<template>
  <section class="mc-card" :class="{ 'mc-card--tight': padded === false }">
    <header v-if="title || $slots.header" class="mc-card__head">
      <slot name="header">
        <h2 class="mc-card__title">{{ title }}</h2>
      </slot>
    </header>
...
.mc-card__body {
  padding: var(--mc-app-pad-card, 1.75rem);
  min-width: 0;
  overflow-x: auto;
}
